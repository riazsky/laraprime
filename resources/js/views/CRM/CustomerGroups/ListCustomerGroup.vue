...
<div>
    ...
    <div class="p-d-flex p-jc-between p-mb-4">
        <h2>List Customer Group</h2>
        <button class="btn btn-success"><router-link to="/create-customer-group" class="text-white">Create Customer Group</router-link></button>
    </div>
    <Card>
      <template #content>
        <DataTable
          :value="ListCustomerGroup" :paginator="true" :rows="10"
...
export default {
  ...
  data() {
    return {
      ...
      items: [
            ...
            {label: 'Create Customer Group', to:"/create-customer-group"},
  ...
  methods: {
        onRowEditInit(event) {
            this.originalRows[event.index] = {...this.ListCustomerGroup[event.index]};
        },
